<?php
  ...
  $consulta = "SELECT * FROM agencias WHERE EMPRESA = 'TEALCA' AND (EMPRESA LIKE '%$term%' 
  ...
  $consulta="SELECT *, MATCH (EMPRESA,COD_AGE,NOM_AGE,DIR_AGE,EST_AGE,CIU_AGE) AGAINST ('$term' IN BOOLEAN MODE) as relevancia
    FROM agencias WHERE MATCH (EMPRESA,COD_AGE,NOM_AGE,DIR_AGE,EST_AGE,CIU_AGE) AGAINST ('$term' IN BOOLEAN MODE ) AND EMPRESA = 'TEALCA' HAVING relevancia > 0.2 ORDER BY relevancia DESC LIMIT 0,10";
  }    
